...
class UartReader(threading.Thread):
    ...
    def __init__(self, port, baud, out_q, debug=False):
        ...
        self._stop_ev = threading.Event(); self.ser=None
    def run(self):
...
class MusicSelectScene:
    ...
    def enter(self):
        ...
        self.choices = [
            Choice("GOLDEN","Golden",     golden, rect_lt, t_g, Hold(self.hold_s, self.timeout)),
            Choice("SODA",  "Soda Pop",   soda,   rect_rt, t_s, Hold(self.hold_s, self.timeout)),
            Choice("COMING1","Coming Soon",ques,  rect_lb, t_c1, Hold(self.hold_s, self.timeout)),
            Choice("COMING2","Coming Soon",ques,  rect_rb, t_c2, Hold(self.hold_s, self.timeout)),
        ]
    ...
    def _pulse(self, name, ts):
        for ch in self.choices:
            if ch.key==name: ch.hold.pulse(ts)

    def handle_event(self, e, now):
        if e.type==pygame.KEYDOWN:
            if e.key==pygame.K_1: self._pulse("GOLDEN", now)
            elif e.key==pygame.K_2: self._pulse("SODA", now)
            elif e.key==pygame.K_3: self._pulse("COMING1", now)
            elif e.key==pygame.K_4: self._pulse("COMING2", now)
    ...
    def _drain_uart(self):
        try:
            while True:
                ...
                su = s.upper()
                if su in ("GOLDEN","G","LT","LEFTTOP","LEFT_TOP"): self._pulse("GOLDEN", ts)
                elif su in ("SODA","SODA_POP","S","RT","RIGHTTOP","RIGHT_TOP"): self._pulse("SODA", ts)
                elif su in ("COMING1","C1","LB","LEFTBOTTOM","LEFT_BOT"): self._pulse("COMING1", ts)
                elif su in ("COMING2","C2","RB","RIGHTBOTTOM","RIGHT_BOT"): self._pulse("COMING2", ts)
                # 필요하면 'QSTICK'을 GOLDEN에 매핑:  if su=="QSTICK": self._pulse("GOLDEN", ts)
        except queue.Empty:
            pass
    ...
    def update(self, dt, now):
        ...
        if self._selected is None:
            for ch in self.choices:
                ...
                if done and self._selected is None:
                    self._selected = ch.key; self._confirm_t = 0.0
        else:
            self._confirm_t += dt
    ...
    def draw(self, screen):
        ...
        now = time.monotonic()
        for ch in self.choices:
            ratio, holding, _ = ch.hold.update(0, now)  # 조회만
            # --- 스케일 계산 (확정 연출 포함) ---
            scale = ch.scale_on_hold if (holding or (self._selected==ch.key)) else 1.0
            if self._selected == ch.key:
                # 단일 스크립트와 동일: 1초 동안 살짝 줄어드는 바운스
                t = 0.0 if self.confirm_show <= 0 else min(1.0, self._confirm_t / self.confirm_show)
