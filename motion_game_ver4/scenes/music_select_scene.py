...
from utils.helpers import _stroke_rect, _glow_rect
import pygame
    ...
    def draw(self, screen):
        ...
        for ch in self.choices:
            ...
            if not ch.enabled:
                ...
                # 박스 테두리 (덮개 위에 그려서 확실히 보이게)
                _box = rect.inflate(8, 8)
                _stroke_rect(screen, _box, (160,160,170), width=4, radius=18)
                continue
            ...
            if self._selected == ch.key:
                col = (255, 215, 0)      # 선택: Gold
            elif holding:
                col = (100, 150, 255)    # 홀드 중: Neon Blue
            else:
                col = (200, 210, 230)    # 기본: Light Gray

            # 박스 테두리 + 은은한 글로우
            _box = rect.inflate(8, 8)
            _glow_rect(screen, _box, col, radius=22, layers=3)
            _stroke_rect(screen, _box, col, width=6, radius=22)
