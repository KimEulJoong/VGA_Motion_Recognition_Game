...
class StartHoldWidget:
    ...
    def __init__(self, screen_size: Tuple[int,int],
        ...
        self._kb_hold = False
        self._uart_sent_flags: dict[bytes, bool] = {}  # 보낸 데이터 기록
    ...
    def handle_event(self, e, now=None):
        # 스페이스/엔터 꾹 누르면 시작 (디버그용)
        if e.type == pygame.KEYDOWN and e.key in (pygame.K_SPACE, pygame.K_RETURN):
            self._kb_hold = True
        elif e.type == pygame.KEYUP and e.key in (pygame.K_SPACE, pygame.K_RETURN):
            self._kb_hold = False

        if self._kb_hold:
            self.hold.on_pulse(time.monotonic() if now is None else now)
    ...
    def update(self, dt: float) -> bool:
        ...
        if getattr(self, "_kb_hold", False):
            self.hold.on_pulse(now)
        if getattr(self, "_mouse_down", False):
            self.hold.on_pulse(now)

        # (UART는 main에서 pump_uart()로 pulse 넣고 있다면 여기선 건드릴 필요 없음)

        ratio, active, just_done = self.hold.update(dt, now)
    ...
    def send_uart(self, data: bytes):
            """UART로 데이터를 보내는 안전한 메서드. 한 번만 전송."""
            if self._uart_sent_flags.get(data, False):
                return  # 이미 보낸 데이터는 다시 보내지 않음

            self._uart_sent_flags[data] = True  # 기록
            ...
            if self.reader and self.reader.ser and self.reader.ser.is_open:
                ...
                print(f"[UART][FAKE TX BYTE] {data.hex()} ({data.decode(errors='ignore')})")

    def reset(self):
        self.hold.state = HoldState()
        self._ratio = 0.0; self._active = False; self._completed = False
        self._uart_sent_flags.clear()  # Reset 시 전송 기록 초기화
