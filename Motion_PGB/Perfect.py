...
char1_path = "./M1.png"        # 1p
perfect_path = "./bad.png" # PERFECT!! 이미지
gear_path = "./gear.png"       # 설정 아이콘
...
draw.rectangle([vga_x0, vga_y0, vga_x1, vga_y1], fill=(0, 0, 0, 255))
...
bg.paste(char1, (220, 450), char1)

# --------------------------
# PERFECT!! 이미지 삽입
# --------------------------
perfect_img = Image.open(perfect_path).convert("RGBA")
vga_w, vga_h = vga_x1 - vga_x0, vga_y1 - vga_y0
perfect_resized = perfect_img.resize((vga_w, vga_h))
bg.paste(perfect_resized, (vga_x0, vga_y0), perfect_resized)

# --------------------------
# 설정 아이콘 삽입 (좌측 상단)
# --------------------------
gear_img = Image.open(gear_path).convert("RGBA").resize((400, 400))
bg.paste(gear_img, (-70, -70), gear_img)
